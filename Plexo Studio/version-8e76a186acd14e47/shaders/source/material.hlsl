...
Surface surfaceShader(SurfaceInput IN, float2 fade2)
{
   ...
   float4 diffuse = sampleFar(DiffuseMap, uv, fadeDiffuse, CFG_FAR_DIFFUSE_CUTOFF);
   
   // ACHTUNG! G(FadeDistance).w has been added to turn on/off the LowQ materials (US23185) FFlag::LowQMaterialsEnable
   // TODO: replace the saturate() with 'fade' when getting rid of the fflag
   diffuse.rgba = lerp(float4(1, 1, 1, 1), diffuse.rgba * (CFG_DIFFUSE_SCALE), saturate1(fade + G(FadeDistance).w) );
